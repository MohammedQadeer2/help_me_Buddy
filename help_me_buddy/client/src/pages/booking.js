...
function Booking() {
  ...
  const handleBooking = async () => {
    ...
    const basePrice = provider.pricePerHour || provider.price || 0;
    ...
    try {
      ...
      if (userStr) {
        ...
        userId = user.id || user._id; // Accommodate both local mock and DB id
      }

      // If we have a backend and userId, we attempt to save the booking
      await createBooking({
        userId,
      ...
      console.warn("Could not save to DB (Continuing locally)", e);
    } finally {
      setLoading(false);
    }
  ...
  const calculateTotal = () => {
    // Add fallback so it handles both API schemas (pricePerHour) and Mock (price)
    const basePrice = provider.pricePerHour || provider.price || 0;
      ...
      <div className="w-full max-w-md sm:max-w-lg md:max-w-2xl lg:max-w-5xl xl:max-w-7xl 2xl:max-w-[1600px] px-4 sm:px-6 lg:px-8 py-6">
        ...
        {!confirmed ? (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            {/* 🧑‍🔧 Provider Summary */}
            <div className="bg-gray-900 border border-gray-800 p-4 rounded-2xl mb-6 flex flex-col sm:flex-row sm:items-center gap-4 shadow-sm">
              <Avatar src={provider.profileImage} size="lg" />
              <div className="flex-1 text-center sm:text-left">
                {/* Simple fallback: Use API data or Mock Data */}
                <h3 className="text-lg font-bold">{provider.name || (provider.userId?.name)}</h3>
                <p className="text-sm text-blue-400">{provider.category?.name || provider.category}</p>
              </div>
              <div className="text-center sm:text-right">
                <p className="text-lg font-bold text-green-400">₹{provider.pricePerHour || provider.price}</p>
            ...
            <div className="bg-gray-900 border border-gray-800 p-5 rounded-2xl mb-6 space-y-4 shadow-sm">
              <h2 className="text-lg md:text-xl font-bold text-gray-200 border-b border-gray-800 pb-2 mb-4 tracking-wide">
                Service Details
              </h2>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 lg:gap-5 mb-4">
                <div>
                  <label className="block mb-1.5 text-xs md:text-sm font-bold text-gray-400 uppercase tracking-wider">Date</label>
                  <input
                    type="date"
                    className="w-full p-2.5 lg:p-3 rounded-lg bg-gray-800 border border-gray-700 text-sm outline-none focus:ring-2 focus:ring-blue-500/50 text-white shadow-inner transition-all hover:bg-gray-800/80 cursor-pointer"
                    value={date}
                    onChange={(e) => setDate(e.target.value)}
                  />
                </div>
                <div>
                  <label className="block mb-1.5 text-xs md:text-sm font-bold text-gray-400 uppercase tracking-wider">Time</label>
                  <input
                    type="time"
                    className="w-full p-2.5 lg:p-3 rounded-lg bg-gray-800 border border-gray-700 text-sm outline-none focus:ring-2 focus:ring-blue-500/50 text-white shadow-inner transition-all hover:bg-gray-800/80 cursor-pointer"
                    value={time}
              ...
              <div>
                <label className="block mb-1.5 text-xs md:text-sm font-bold text-gray-400 uppercase tracking-wider">Service Address</label>
                <input
                  type="text"
                  placeholder="Enter full address..."
                  className="w-full p-2.5 lg:p-3 rounded-lg bg-gray-800 border border-gray-700 text-sm outline-none focus:ring-2 focus:ring-blue-500/50 text-white placeholder-gray-500/50 shadow-inner transition-all"
                  value={address}
            ...
            {/* Price Preview */}
            <div className="bg-gray-900/50 border border-gray-800 p-4 md:p-6 rounded-xl mb-6 shadow-inner">
              {/* Added safe base price */}
              <div className="flex justify-between text-sm md:text-base text-gray-400 mb-1.5"><span>Base Price:</span><span className="text-gray-200">₹{provider.pricePerHour || provider.price || 0}</span></div>
              <div className="flex justify-between text-sm md:text-base text-gray-400 mb-1.5"><span>Tax (5%):</span><span className="text-gray-200">₹{Math.round((provider.pricePerHour || provider.price || 0) * 0.05)}</span></div>
              <div className="flex justify-between text-sm md:text-base text-gray-400 mb-2"><span>Platform Fee:</span><span className="text-xs md:text-sm text-gray-500">(Paid by provider)</span></div>
              <div className="w-full border-t border-gray-700/50 my-3"></div>
              <div className="flex justify-between font-extrabold text-lg md:text-xl lg:text-2xl mt-3"><span>Total You Pay:</span><span className="text-white bg-green-500/20 px-3 py-1 rounded-md border border-green-500/30 text-green-400">₹{calculateTotal()}</span></div>
              <p className="text-xs md:text-sm text-center mt-4 text-gray-500 font-medium bg-gray-900/40 p-2.5 rounded-lg border border-gray-800/50">
            ...
            <Button
              ...
              disabled={loading}
              variant="primary"
              className="py-3 lg:py-4 text-base md:text-lg font-extrabold shadow-lg shadow-blue-500/20 w-full rounded-xl tracking-wide uppercase hover:scale-[1.02]"
            >
              {loading
                ? "Processing..."
          ...
          >
            ...
            <p className="text-lg md:text-xl text-gray-300 mb-6 md:mb-8 font-medium">
              <span className="font-bold text-white">{provider.name || (provider.userId && provider.userId.name)}</span> will arrive on <span className="font-bold text-white">{date}</span> at <span className="font-bold text-white">{time}</span>.
            </p>
